...
public class TreParti extends  PartiCorpo implements Interfaccia{

    @Override
    public Map assegnaEsercizi(Esercizio product) {

        for (int i = 0; i < product.getSize(); i++) {
            if (i >= (product.getSize()-6)) {
                    ...
                    case 7:
                        if (!product.getTipo().equals("Scheda Personalizzata")) {
                            ...
                            if (i<=3) {
                                ...
                                schedaPersonalizzata(i, 5, 3, product.getTipo(), product.getShuffle(), product.getListaEsercizi(), product.getListaPartiCorpo(), product.getListaMetRep());
                            }
                        }   
                        break;
                    default:
